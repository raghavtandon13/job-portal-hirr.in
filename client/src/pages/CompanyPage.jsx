...
const CompanyPage = () => {
  ...
  return (
    <>
      {isLoggedIn() ? <Navbar buttonLink="/org/home" buttonLabel="Home" button2Link="/org/profile" button2Label="Profile" funcBtnName="Logout" funcBtn={handleLogout} dropdownName={"Settings"} dropdown1="option #1" dropdown2="option #2" dropdown2Link="#" dropdown1Link="#" /> : <Navbar buttonLink="/login" buttonLabel="Login" button2Link="/signup" button2Label="Register" funcBtnName="About Us" />}
      <div className="com-cont">
        <OrgProfile useNewApi={true} companyId={companyId} />
        <div className="card-container">{responseData.length === 0 ? <p className="no-res">No results found.</p> : responseData.map((item) => <Card key={item._id} data={item} />)}</div>
      </div>
    </>
  );
};
